...
export const AdminAllFeedback: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError(null);
      try {
        const res = await feedbackApi.getAllResponses();
        setResponses(res as AnyResponse[]);
      } catch (e) {
        setError(e instanceof Error ? e.message : 'Failed to load all feedback');
      } finally {
        setLoading(false);
      }
    };
  ...
  const grouped = useMemo(() => {
    ...
    for (const r of responses) {
      ...
      const formTitle = r.form_id.form_title || 'Form';
      const createdByName = r.form_id.created_by_name || null;
  ...
  const studentLabel = (resp: AnyResponse) => {
    if (resp.is_anonymous) return 'Anonymous';
    if (resp.user_id && typeof resp.user_id === 'object') {
      return resp.user_id.full_name || resp.user_id.email || 'Student';
    }
    return 'Student';
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-600 to-purple-700 rounded-lg text-white p-6">
        ...
        <p className="mt-2 opacity-90">All responses across all forms (admin).</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="text-center">
          ...
          <p className="text-sm text-gray-600 mt-2">
            {grouped[0]?.overallAvgRating ?? 'N/A'}
          </p>
        </Card>
      </div>

      <Card title="All feedback (click to open)" subtitle="Click a feedback form to view responses">
        {loading ? (
          <div className="text-center py-8 text-gray-500">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-purple-600 mx-auto" />
            <p className="mt-2">Loading all feedback...</p>
          </div>
        ) : error ? (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        {g.createdByName ? <>Created by: {g.createdByName} | </> : null}
                    ...
                    <div className="p-4 border-t border-gray-200 bg-white">
                      ...
                      const answerEntries = entries
                        .map((e) => ({
                          student: studentLabel(e.resp),
                          answer:
                            e.ans?.answer_text != null && String(e.ans.answer_text).trim()
